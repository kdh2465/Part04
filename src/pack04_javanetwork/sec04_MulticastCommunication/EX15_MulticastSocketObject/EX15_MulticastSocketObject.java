...
public class EX15_MulticastSocketObject {
	public static void main(String[] args) {
		...
		try {
			...
			mcs3 = new MulticastSocket(new InetSocketAddress("localhost",10000));
		...
		try {
			System.out.println(mcs1.getTimeToLive()); //1
			mcs1.setTimeToLive(50);
			System.out.println(mcs1.getTimeToLive()); //50
		...
		try {
			...
			DatagramPacket receivedPacket = new DatagramPacket(receivedData, receivedData.length, InetAddress.getByName("234.234.234.234"), 20000);
			mcs1.setSoTimeout(1000);
			mcs1.receive(receivedPacket);
